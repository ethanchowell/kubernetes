...
func NewCmdTopPod(f cmdutil.Factory, o *TopPodOptions, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().BoolVar(&o.Enumerate, "enumerate", o.Enumerate, "If present, list the cpu and memory requests and limits defined in the manifest.")
	return cmd
}
...
func (o TopPodOptions) RunTopPod() error {
	...
	if len(metrics.Items) == 0 {
		...
		err := verifyEmptyMetrics(o, labelSelector, fieldSelector, pods)
		if err != nil {
			return err
		}
...
func verifyEmptyMetrics(o TopPodOptions, labelSelector labels.Selector, fieldSelector fields.Selector, pods *corev1.PodList) error {
	var err error
	if len(pods.Items) > 0 {
		...
		return verifyEmptyMetrics(o, labelSelector, fieldSelector, pods)
	}
